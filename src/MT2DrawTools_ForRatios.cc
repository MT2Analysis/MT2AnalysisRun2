...
MT2DrawTools_ForRatios::MT2DrawTools_ForRatios( const std::string& outDir, float lumi ) {
...
void MT2DrawTools_ForRatios::set_data( MT2Analysis<MT2EstimateTree>* data ) {
...
void MT2DrawTools_ForRatios::set_lumi( float lumi) { 

  std::cout << "[MT2DrawTools_ForRatios] Setting lumi to: " << lumi<< std::endl;
  lumi_ = lumi; 
...
void MT2DrawTools_ForRatios::set_year(int year) { 

  std::cout << "[MT2DrawTools_ForRatios] Setting year to: " << year<< std::endl;
  year_ = year; 
...
//  }
  ...
  for( std::set<MT2Region>::iterator iMT2 = MT2Regions.begin(); iMT2!=MT2Regions.end(); ++iMT2 ) {
    ...
    if( tree_data ) {
      ...
      tree_data->Project( "h1_data", varName.c_str(), Form("%f*weight*(%s)", lumi_, selection1.c_str()) );
      //tree_data->Project( "h1_data", varName.c_str(), Form("%f*(%s)", data_->getWeight(), selection.c_str()) );
      if( addOverflow_ )
        MT2DrawTools_ForRatios::addOverflowSingleHisto(h1_data);
      gr_data = MT2DrawTools_ForRatios::getPoissonGraph(h1_data, false, "binWidth");
    ...
    for( unsigned i=0; i<mc_->size(); ++i ) { 
      ...
      h1_mc->Sumw2();
      if( selection2!="" )
        tree_mc->Project( thisName.c_str(), varName.c_str(), Form("%f*weight*(%s)", lumi_, selection2.c_str()) );
      ...
      if( addOverflow_ )
        MT2DrawTools_ForRatios::addOverflowSingleHisto(h1_mc);
    ...
    if( data_ ) g_ratio = MT2DrawTools_ForRatios::getRatioGraph(h1_data, mcBand);
 ...
 //   }
    ...
    for( unsigned i=0; i<histos_mc.size(); ++i ) {  
      ...
      legend->AddEntry( histos_mc[i], sampleName + " CR " + year, "F" );
    ...
    (data_) ? MT2DrawTools_ForRatios::addLabels( (TCanvas*)pad1, lumi_, CMStext.c_str() ) : MT2DrawTools_ForRatios::addLabels( (TCanvas*)c1, lumi_, "CMS Simulation"); 
    ...
    (data_) ? MT2DrawTools_ForRatios::addLabels( (TCanvas*)pad1_log, lumi_, CMStext.c_str() ) : MT2DrawTools_ForRatios::addLabels( (TCanvas*)c1_log, lumi_, "CMS Simulation"); 
