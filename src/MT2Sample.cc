...
std::vector<MT2Sample> MT2Sample::loadSamples(const std::string& filename,const std::string& filter, int idMin, int idMax, int isETH){
  return loadSamples(filename,"",filter,idMin,idMax,isETH);

}

std::vector<MT2Sample> MT2Sample::loadSamples(const std::string& filename,const std::string& scorefilename, const std::string& filter, int idMin, int idMax, int isETH) {
  ...
  std::ifstream INscore(scorefilename.c_str());

  std::cout << "------------------------------------" << std::endl;
  std::cout << "Sample File  " << filename << std::endl;
  if(scorefilename!="") std::cout << "Score File  " << scorefilename << std::endl;
  int counter(0);
  TString fPath;
  ...
  if (scorefilename!=""){
    while( INscore.getline(buffer, 1500, '\n') ) {
      ...
      TString samplescore(sampleFilePath);
      ...
      scorefilenamelist.push_back(rootFileName);
      scorefilefullnamelist.push_back(samplescore);
//      cout<<"scorefilename "<<rootFileName<<endl;
    }
  }
  ...
  while( IN.getline(buffer, 1500, '\n') ) {
    ...
    if(scorefilename!=""){
      int i;
      for(i=0;i<scorefilenamelist.size();i++){
        TString rootFileNameinst(pathDirs[pathDirs.size()-2]);
        if((scorefilenamelist[i]).Contains(rootFileNameinst.ReplaceAll(".root","").Data())){
          tree->AddFriend("mt2_friend",scorefilefullnamelist[i].Data());
          s.score=scorefilefullnamelist[i].Data();
//          cout<<"file "<<rootFileName<<" add score "<<scorefilenamelist[i]<<endl;
          break;
        }
      }
      if(i==scorefilenamelist.size()) cout<< "[MT2Sample::loadSamples] Cannot find score for "<<s.file<<endl;
    }
    int evt_id;
...
//    if(s.id<=0){
      ...
      else if( rootFileName.Contains("qcd") )          s.id = 10;//qcd background merged
      ...
      // singletop merged
      else if (sampleFilePath_tstr.Contains("singletop"))               s.id=407;
