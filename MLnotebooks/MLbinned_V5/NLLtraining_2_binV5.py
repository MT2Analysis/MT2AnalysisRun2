...
if signaltype=="T1bbbb":
    models_forbin=initialize.models_T1bbbb_s
elif signaltype=="T1qqqq":
    models_forbin=initialize.models_T1qqqq_s
elif signaltype=="T2bb":
    models_forbin=initialize.models_T2bb_s
elif signaltype=="T2qq":
    models_forbin=initialize.models_T2qq_s
...
for j in range(len(models_forbin['mass'])):
    ...
    if len(models_forbin['binindex'][j])==0:
        continue
...
#    models_forbin['significance_plots'].append([])
    for i in models_forbin['binindex'][j]:
        ...
        if j==0 and signaltype=="T1bbbb":
            X1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1800)&(X1[b'GenSusyMScan2']>=1200)][(features_train+[b'evt_scale1fb',b'GenSusyMScan1',b'GenSusyMScan2',b'absweight',b'bkgtype',b'w2'])[:]]
            y1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1800)&(X1[b'GenSusyMScan2']>=1200)][b'sigtag']
        if j==0 and signaltype=="T1qqqq":
            X1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1800)&(X1[b'GenSusyMScan2']>=1000)][(features_train+[b'evt_scale1fb',b'GenSusyMScan1',b'GenSusyMScan2',b'absweight',b'bkgtype',b'w2'])[:]]
            y1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1800)&(X1[b'GenSusyMScan2']>=1000)][b'sigtag']
        if j==3 and signaltype=="T2bb":
            X1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1000)&(X1[b'GenSusyMScan2']>=500)][(features_train+[b'evt_scale1fb',b'GenSusyMScan1',b'GenSusyMScan2',b'absweight',b'bkgtype',b'w2'])[:]]
            y1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1000)&(X1[b'GenSusyMScan2']>=500)][b'sigtag']
        if j==3 and signaltype=="T2qq":
            X1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1400)&(X1[b'GenSusyMScan2']>=700)][(features_train+[b'evt_scale1fb',b'GenSusyMScan1',b'GenSusyMScan2',b'absweight',b'bkgtype',b'w2'])[:]]
            y1=datasig_ML_bin[i][(datasig_ML_bin[i][b'GenSusyMScan1']<=1400)&(X1[b'GenSusyMScan2']>=700)][b'sigtag']
...
#            valindex.append([list(X0_val.index.values),list(X1_val.index.values)])
    ...
    #res_data=pd.concat([data_input[i][1].iloc[choices,:],data_input[i][0]])
            ...
            filepath="weights_"+signaltype+"_bin_"+str(i)+"_s.best.hdf5"
            ...
            histroyplot.savefig("learning_curve_"+signaltype+"_bin"+str(i)+"_s.png")
            histroyplot.savefig("learning_curve_"+signaltype+"_bin"+str(i)+"_s.pdf")
...
#            if zlldata[i].shape[0]:
            ...
            figcompare = methods.plot_train_val_compare(train0,train1,val0,val1,'train and validation comparison in bin '+str(i)+' ht '+str(binning.selections[i][0][0])
                       +"-"+str(binning.selections[i][0][1])+" njet "+str(binning.selections[i][1][0])+"-"+str(binning.selections[i][1][1]-1)
                       +" nbjet "+str(binning.selections[i][2][0])+"-"+str(binning.selections[i][2][1]-1))
            figcompare.savefig("train_val_compare_"+signaltype+"_bin"+str(i)+"_s.png")
            figcompare.savefig("train_val_compare_"+signaltype+"_bin"+str(i)+"_s.pdf")
 #           figcompare.savefig("train_val_compare_bin"+str(i)+".eps")
            ...
            figtest = methods.plot_sig_bkg_compare(datasig_test_bin[i],databkg_test_bin[i],m1,m2,'signal and background comparison(test set) in bin '
                       ...
                       +"-"+str(binning.selections[i][0][1])+" njet "+str(binning.selections[i][1][0])+"-"+str(binning.selections[i][1][1]-1)
                       +" nbjet "+str(binning.selections[i][2][0])+"-"+str(binning.selections[i][2][1]-1))
            figtest.savefig("sig_bkg_compare_"+signaltype+"_bin"+str(i)+"_mass_"+str(m1)+"_"+str(m2)+"_s.png")
            figtest.savefig("sig_bkg_compare_"+signaltype+"_bin"+str(i)+"_mass_"+str(m1)+"_"+str(m2)+"_s.pdf")
#            figtest.savefig("sig_bkg_compare_bin"+str(i)+".eps")
            ...
            fig_mt2scorecorr=methods.plot_score_mt2_corr(datasig_test_bin[i],databkg_test_bin[i],'MT2 score correlation check (test set) \nin bin '+str(i)+' ht '+str(binning.selections[i][0][0])
                       +"-"+str(binning.selections[i][0][1])+" njet "+str(binning.selections[i][1][0])+"-"+str(binning.selections[i][1][1]-1)
                       +" nbjet "+str(binning.selections[i][2][0])+"-"+str(binning.selections[i][2][1]-1))
            fig_mt2scorecorr.savefig("mt2scorecorr_"+signaltype+"_bin"+str(i)+"_s.png")
            fig_mt2scorecorr.savefig("mt2scorecorr_"+signaltype+"_bin"+str(i)+"_s.pdf")
            ...
            figllep=methods.plot_CR_llep(llepMC[i],llepdata[i],'data MC comparison of lost lepton CR in bin '+str(i)+' ht '+str(binning.selections[i][0][0])
                       +"-"+str(binning.selections[i][0][1])+" njet "+str(binning.selections[i][1][0])+"-"+str(binning.selections[i][1][1]-1)
                       +" nbjet "+str(binning.selections[i][2][0])+"-"+str(binning.selections[i][2][1]-1))
            figllep.savefig("llepCR_"+signaltype+"_bin"+str(i)+"_s.png")
            figllep.savefig("llepCR_"+signaltype+"_bin"+str(i)+"_s.pdf")
...
#            if zlldata[i].shape[0]*zllMC[i].shape[0]:
            figzll=methods.plot_CR_zll(zllMC[i],zlldata[i],'data MC comparison of zll CR in bin '+str(i)+' zll_ht '+str(binning.selections[i][0][0])
                       +"-"+str(binning.selections[i][0][1])+" njet "+str(binning.selections[i][1][0])+"-"+str(binning.selections[i][1][1]-1)
                       +" nbjet "+str(binning.selections[i][2][0])+"-"+str(binning.selections[i][2][1]-1))
            figzll.savefig("zllCR_"+signaltype+"_bin"+str(i)+"_s.png")
            figzll.savefig("zllCR_"+signaltype+"_bin"+str(i)+"_s.pdf")
...
#        if zlldata[i].shape[0]*zllMC[i].shape[0]:
        ...
        figmidsig.savefig("significance_"+signaltype+"_bin_"+str(i)+"_mass_"+str(m1)+"_"+str(m2)+"_s.png")
        figmidsig.savefig("significance_"+signaltype+"_bin_"+str(i)+"_mass_"+str(m1)+"_"+str(m2)+"_s.pdf")
#        figmidsig.savefig("significance_bin_"+str(i)+"_mass_"+str(m1)+"_"+str(m2)+".eps")
#        else: 
#            midsig=[0]
#            models_forbin['significance'][j].append(midsig)
        print("finished plotting")
np.save("significance_"+signaltype+"_output_s.npy",models_forbin)
