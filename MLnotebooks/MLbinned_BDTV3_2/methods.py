...
def plot_hyperparameter(score_train,score_va,xlabel,title,x_range):
    fig = plt.figure(figsize=(20,10))
    ax  = fig.add_subplot(111)
    for fold in range(len(score_va)):
        ax.plot(x_range,score_va[fold],'--',label='Validation_'+str(fold+1));
        ax.plot(x_range,score_train[fold],label='Training_'+str(fold+1));
    ax.legend(loc='center left',bbox_to_anchor=(1, 0.5),prop={'size': 14})
    ax.set_xlabel(xlabel,fontsize=20)
    ax.set_ylabel("1-MSE/Var",fontsize=20)
    ax.tick_params(axis='both', which='major', labelsize=16)
    ax.tick_params(axis='both', which='minor', labelsize=16)
    ax.set_title(title,size=22)
    return fig

def plot_hyperparameter_average(score_train_average,score_va_average,score_train_std,score_va_std,xlabel,title,x_range):
    fig = plt.figure(figsize=(20,10))
    ax  = fig.add_subplot(111)
    ax.plot(x_range,score_va_average,'--',label='Validation',color='blue');
    ax.plot(x_range,score_train_average,label='Training',color='orange');
    ax.errorbar(x_range,score_va_average,yerr=score_va_std,xerr=None,ecolor='blue',c='blue',fmt='o')
    ax.errorbar(x_range,score_train_average,yerr=score_train_std,xerr=None,ecolor='orange',c='orange',fmt='o')
...
def plot_hyperparameter_average_plustest(score_train_average,score_va_average,score_test_average,score_train_std,score_va_std,score_test_std,xlabel,title,x_range):
    fig = plt.figure(figsize=(20,10))
    ax  = fig.add_subplot(111)
    ax.plot(x_range,score_va_average,'--',label='Validation',color='blue');
    ax.plot(x_range,score_train_average,label='Training',color='orange');
    ax.plot(x_range,score_test_average,':',label='Test',color='red');
    ax.errorbar(x_range,score_va_average,yerr=score_va_std,xerr=None,ecolor='blue',c='blue',fmt='o')
    ax.errorbar(x_range,score_train_average,yerr=score_train_std,xerr=None,ecolor='orange',c='orange',fmt='o')
    ax.errorbar(x_range,score_test_average,yerr=score_test_std,xerr=None,ecolor='red',c='red',fmt='o')
    ax.legend(loc='best',prop={'size': 16})
    ax.set_xlabel(xlabel,fontsize=20)
    ax.set_ylabel("1-MSE/Var",fontsize=20)
    ax.tick_params(axis='both', which='major', labelsize=16)
...
def plot_train_val_compare(train_bkg,train_sig,val_bkg,val_sig,title):
    ...
    ax.errorbar(bincenters,hist_val_bkg,yerr=err_val_bkg,xerr=None,ecolor='blue',c='blue',fmt='o',label='test background in SR')
    ax.errorbar(bincenters,hist_val_sig,yerr=err_val_sig,xerr=None,ecolor='red',c='red',fmt='o',label='test inclusive signal')
