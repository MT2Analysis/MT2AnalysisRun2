...
class MT2LeptonSFTool {
  ...
  public:
    ...
    bool setElHistos( std::string, std::string, std::string);
    bool setMuHistos( std::string, std::string, std::string );
    //bool setVetoEffHistos( std::string filename);

    // get SF methods
    lepSF getElSF(int year, float pt, float eta);
    //lepSF getMuSF(int year, float pt, float eta);
