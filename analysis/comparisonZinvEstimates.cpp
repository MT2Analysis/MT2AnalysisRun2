...
void plotEstimate(){
  ...
  TFile* file = TFile::Open("/t3home/anlyon/CMSSW_8_0_12/src/myMT2Analysis_2016compatible/analysis/EventYields_dataETH_SnTMC_35p9ifb/zllControlRegion/mc_forZinvEst.root");
  ...
  string fileR = "/t3home/anlyon/CMSSW_8_0_12/src/myMT2Analysis_2016compatible/analysis/EventYields_dataETH_SnTMC_35p9ifb/zllControlRegion/mc_forZinvEst.root";
  ...
  for(std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR){
	...
	if(NjMax != "-1"){
      ...
      if(p==1){
	tableLeg[legIndex-1] = new TLegend(0.65, 0.7, 0.85, 0.9);
        if(NbMaxInt == -1){
	...
	tableLeg[legIndex-1] -> SetFillColorAlpha(0, 1);
	...
	if((htMinInt == htMin_precedingRegion) && (htMaxInt == htMax_precedingRegion) && (NjMinInt == NjMin_precedingRegion) && (NjMaxInt == NjMax_precedingRegion)){
	  tableLeg[legIndex-1] = new TLegend(0.65, 0.7-i, 0.85, 0.9-i);
	  if(NbMaxInt == -1){
	    tableLeg[legIndex-1] -> AddEntry(hist, "b" + nb + "toInf");
	  }
	  else{
	...
	c->SaveAs("EventYields_dataETH_SnTMC_35p9ifb/plotsZllEstimates/MT2vsNb/" + n + "_normalized.pdf");
	if(legIndex>fileIndex){
	  remove("EventYields_dataETH_SnTMC_35p9ifb/plotsZllEstimates/MT2vsNb/" + n_previous + "_normalized.pdf");
	}
      }
...
void plotEstimateVSNb(){
 ...
 hist1 = estimate1->get(*iR)->yield;
  ...
  for(std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR){
	...
	tableLeg[legIndex-1] = new TLegend(0.65, 0.7, 0.85, 0.9);
        if(NbMaxInt == -1){
	  tableLeg[legIndex-1] -> AddEntry(hist, "n" + nb + "toInf");
        }
	else{
	  tableLeg[legIndex-1] -> AddEntry(hist, "n" + nb);
        }
	tableLeg[legIndex-1] -> SetTextSize(0.04);
	tableLeg[legIndex-1] -> SetLineColor(0);
	tableLeg[legIndex-1] -> SetFillColorAlpha(0, 1);
