...
int main( int argc, char* argv[] ) {
  ...
  if( argc==1 ) {
    ...
    std::cout << "Usage: ./printLatexBGTable_2016 [dir]" << std::endl;
    exit(191);
  ...
  for( std::set<MT2Region>::iterator iR = regions.begin(); iR != regions.end(); ++iR ){
	...
	continue;
      ...
      if( iR->nJetsMax()==1 )
	tableName = std::string(Form("%s/datacard_templates_combined/table_%s_m0toInf.txt", dir.c_str(), iR->getName().c_str()) );
      else{
	if( iBin < nBins-1 )
	  tableName = std::string(Form("%s/datacard_templates_combined/table_%s_m%.0lfto%.0lf.txt", dir.c_str(), iR->getName().c_str(), bins[iBin], bins[iBin+1]) );
	else 
	  tableName = std::string(Form("%s/datacard_templates_combined/table_%s_m%.0lftoInf.txt", dir.c_str(), iR->getName().c_str(), bins[iBin] ));
      }
