...
int main( int argc, char* argv[] ) {
  ...
  std::vector< MT2Analysis<MT2EstimateTree>* > mcSR_wjets_;
  std::vector< MT2Analysis<MT2EstimateTree>* > mcSR_top_;
  std::vector< MT2Analysis<MT2EstimateTree>* > mcSR_llep_;
  std::vector< MT2Analysis<MT2EstimateTree>* > mcSR_zjets_;
  mcSR_wjets_.push_back(mcSR_wjets);
  mcSR_top_.push_back(mcSR_top);
  mcSR_llep_.push_back(mcSR_llep);
  mcSR_zjets_.push_back(mcSR_zjets);
  ...
  std::vector<float> lumi = {cfg.lumi()};
  std::vector<int>   year = {cfg.year()};

  MT2DrawTools_ForRatios dt_wjets(plotsDir_wjets, lumi);
  dt_wjets.set_shapeNorm(shapeNorm);
  dt_wjets.set_data(&mcSR_wjets_);
  dt_wjets.set_mc(&mcCR_wjets_);
  dt_wjets.set_lumi(lumi);
  dt_wjets.set_year(year);

  MT2DrawTools_ForRatios dt_top(plotsDir_top, lumi);
  dt_top.set_shapeNorm(shapeNorm);
  dt_top.set_data(&mcSR_top_);
  dt_top.set_mc(&mcCR_top_);
  dt_top.set_lumi(lumi);
  dt_top.set_year(year);
