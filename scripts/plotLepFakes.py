...
if __name__ == "__main__":
  ...
  h_recoLep_all_phi_eta = ROOT.TH2F("h_recoLep_all_phi_eta", "h_recoLep_all_phi_eta", 32, -3.2, 3.2, 30, -3., 3.)
  #h_recoLep_all_phi_eta_tightId = ROOT.TH2F("h_recoLep_all_phi_eta_tightId", "h_recoLep_all_phi_eta_tightId", 32, -3.2, 3.2, 30, -3., 3.)
  h_recoLep_ele_phi_eta = ROOT.TH2F("h_recoLep_ele_phi_eta", "h_recoLep_ele_phi_eta", 32, -3.2, 3.2, 30, -3., 3.)
  h_recoLep_mu_phi_eta = ROOT.TH2F("h_recoLep_mu_phi_eta", "h_recoLep_mu_phi_eta", 32, -3.2, 3.2, 30, -3., 3.)

  h_recoLep_ele_phi_eta_tightId = ROOT.TH2F("h_recoLep_ele_phi_eta_tightId", "h_recoLep_ele_phi_eta_tightId", 32, -3.2, 3.2, 30, -3., 3.)
  h_recoLep_ele_phi_eta_mediumId = ROOT.TH2F("h_recoLep_ele_phi_eta_mediumId", "h_recoLep_ele_phi_eta_mediumId", 32, -3.2, 3.2, 30, -3., 3.)

  histos = [h_recoLep_all_phi_eta, h_recoLep_ele_phi_eta, h_recoLep_mu_phi_eta, h_recoLep_ele_phi_eta_mediumId, h_recoLep_ele_phi_eta_tightId]

  lepSels = ["(1)", "(abs(lep_pdgId[0])==11)",  "(abs(lep_pdgId[0])==13)", "(abs(lep_pdgId[0])==11 && lep_id[0]>=3)", "(abs(lep_pdgId[0])==11  && lep_id[0] >=4)" ]

  lepType = ["reco lepton", "reco electron", "reco muon",  "reco electron", "reco electron"]

  zMaxes = [150, 80, 80, 80, 80]
  ...
  kinematics = "(ht > 250 && met_pt>250  && nJet30>1 && deltaPhiMin>0.3 && diffMetMht<0.5*met_pt && mt2>200. && nRecoLepLowMT==1 && nLepHighMT==0)"
  ...
  for i,h in enumerate(histos):
    ...
    chain.Project(h.GetName(), "lep_eta[0]:lep_phi[0]",  sel, "colz" )
