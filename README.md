...
root preselection_forML_reducefeature_1.cpp
root preselection_forML_reducefeature_llcontrol.cpp
root preselection_forML_reducefeature_zinvcontrol.cpp
bash dataskim_control_signal.sh
```
Then log out, and enter the system again with conda enviroment tensorflow\_cpu, to transfer the skimmed .root files into .npy files. Also remember to change the file paths. Under the same directory 
```
python preselection_root_to_npy.py
```
### Machine learning training
Then you can do the machine learning training. It requires the gpu usage
```
conda activate tf-gpu
...
Then you can evaluated the skimmed ntuples with the models. Before running you need to change the file paths in the script `submitbatch_evaluate.py` (it reads the files in the path of $directory and produce score files in the path of $targetdir)

```
python submitbatch_evaluate.py
```
...
```
conda activate tensorflow_cpu
bash createfriendtree_T1bbbb.sh
bash createfriendtree_T1qqqq.sh
bash createfriendtree_T2bb.sh
bash createfriendtree_T2qq.sh
```
You may also need to change the paths of the score files in the directory `samples`. In the analysis the program matches the names of score files in the `samples/*.dat` files to those of the ntuples. A script `createscoresamples.py` in the main directory may help you create the `*.dat` files conveniently, if you want to create a new optimization version.
...
First you need to make the regions selections and compute the yields. Examples of `<cfg-file-name>` for ML optimization: `config_MLoptimize_<YEAR>_For<SIGNAL>_<VERSION>_<SAMPLENUMBER>`, where `<YEAR>`= 2016, 2017, 2018, `<SIGNAL>`= T1bbbb, T1qqqq, T2bb, T2qq, `<VERSION>`= BDTV3_2, binV10_2, incluV3, `<SAMPLENUMBER>`= 1, 5, 10.
...
To run the analysis automatically, use `launch_analysis_ML_<SIGNAL>_<VERSION>_<SAMPLENUMBER>.sh`.

Open the file, and set the parameters in the user's decision board according to your wish.

Once done, do
```
source launch_analysis_ML_<SIGNAL>_<VERSION>_<SAMPLENUMBER>.sh
```
...
```
hadd EventYields_<cfg-file-name16>/analyses_signals_merged.root EventYields_<cfg-file-name16>/analyses.root EventYields\_<cfg-file-name17>/analyses.root EventYields_<cfg-file-name18>/analyses.root
```
...
```
./createDatacards_combined config_MLoptimize_2016_ForT2qq_BDTV3_2_1 config_MLoptimize_2017_ForT2qq_BDTV3_2_1 config_MLoptimize_2018_ForT2qq_BDTV3_2_1  T2qq 300 305 200 205 pippo  # for single mass point
./createDatacards_combined config_MLoptimize_2016_ForT2qq_BDTV3_2_1 config_MLoptimize_2017_ForT2qq_BDTV3_2_1 config_MLoptimize_2018_ForT2qq_BDTV3_2_1 T2qq 1000 1105 200 255      # for mass scan
```
